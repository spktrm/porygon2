...
export class GameServer {
    ...
    private assignPlayerToGame(
        ...
        if (gameWorker) {
            ...
            this.logger.debug(`Assigned player ${playerId} to game ${gameId}`);
        } else {
            this.logger.error(
    ...
    private findAvailableWorker(gameId: number): GameWorker | null {
        ...
        if (this.workers.length < this.maxWorkers) {
            ...
            this.workers.sort((a, b) => a.gameCount - b.gameCount);
            const leastLoadedWorker = this.workers[0];

            if (leastLoadedWorker.gameCount < this.maxGamesPerWorker) {
                this.logger.info(
