...
# ---- the consequence model (rl/model/consequence.py) ----------------------
# Both losses are pooled per row group and divided by a FIXED scale from
# config -- never by the batch's own magnitude. The first version divided by
# the stop-gradient pooled true change, and the trunk gamed it: inflating its
# rows' step-to-step change (field rows 7.5x) grew the denominator faster than
# the error, so the ratio fell with nothing learned (LESSONS "Step 4
# verdict"). Under a fixed scale, a bigger true change can only cost. The copy
# predictor no longer scores 1 by construction; its score under the same scale
# is logged beside each loss, and that gauge IS the inflation watch.
CONSEQUENCE_NORM_EPS = 1e-3


def smoothed_norm(vector: jax.Array) -> jax.Array:
    """sqrt(|v|^2 + eps^2) - eps: exactly 0 at v = 0 with a zero (finite)
    gradient there. The plain norm's gradient is 0/0 at 0, and the sampler's
    two draws ARE equal at init (zero-init out_proj) -- the non-finite gate
    would then skip every update and the parameters could never move."""
    return (
        jnp.sqrt(jnp.square(vector).sum(axis=-1) + CONSEQUENCE_NORM_EPS**2)
        - CONSEQUENCE_NORM_EPS
    )


def pooled_group_mean(
    values: jax.Array, mask: jax.Array, group_ids: jax.Array, num_groups: int
) -> tuple[jax.Array, jax.Array]:
    """The masked mean of (..., rows) `values` per row group, and each
    group's count. A group with no valid row reads 0."""
    membership = jax.nn.one_hot(group_ids, num_groups, dtype=jnp.float32)
    weight = mask.astype(jnp.float32)[..., None] * membership
    axes = tuple(range(weight.ndim - 1))
    count = weight.sum(axis=axes)
    return (values[..., None] * weight).sum(axis=axes) / jnp.maximum(count, 1.0), count


def _over_present_groups(per_group: jax.Array, count: jax.Array) -> jax.Array:
    present = (count > 0).astype(jnp.float32)
    return (per_group * present).sum() / jnp.maximum(present.sum(), 1.0)


def consequence_mean_loss(
    predicted_next: jax.Array,
    now: jax.Array,
    real_next: jax.Array,
    mask: jax.Array,
    group_ids: jax.Array,
    ...
    """SQUARED error of the predicted next rows over `scales`^2. Squared,
    because an unsquared norm's optimum is the conditional median, which for a
    minority outcome is the copy predictor -- the gradient would vanish on
    exactly the rare consequences this head exists for."""
    predicted_next = predicted_next.astype(jnp.float32)
    real_next = real_next.astype(jnp.float32)
    now = now.astype(jnp.float32)
    error, count = pooled_group_mean(
        jnp.square(predicted_next - real_next).sum(axis=-1),
        mask,
        group_ids,
        scales.shape[0],
    )
    copy_error, _ = pooled_group_mean(
        jnp.square(now - real_next).sum(axis=-1), mask, group_ids, scales.shape[0]
    )
    ratio = error / jnp.square(scales)
    return dict(
        loss=_over_present_groups(ratio, count),
        ratio=ratio,
        copy_ratio=copy_error / jnp.square(scales),
    )


def consequence_energy_loss(
    sampled_next: jax.Array,
    now: jax.Array,
    real_next: jax.Array,
    mask: jax.Array,
    group_ids: jax.Array,
    ...
    """The energy score of `sampled_next` (draws, ..., rows, dim) against the
    real next rows, a strictly proper scoring rule: mean distance to the truth
    minus half the mean distance between draws, over `scales`."""
    sampled_next = sampled_next.astype(jnp.float32)
    real_next = real_next.astype(jnp.float32)
    now = now.astype(jnp.float32)
    num_draws = sampled_next.shape[0]
    num_groups = scales.shape[0]
    skill = smoothed_norm(sampled_next - real_next[None]).mean(axis=0)
    between = smoothed_norm(sampled_next[:, None] - sampled_next[None, :])
    spread = between.sum(axis=(0, 1)) / (num_draws * (num_draws - 1))
    score, count = pooled_group_mean(skill - 0.5 * spread, mask, group_ids, num_groups)
    pooled_skill, _ = pooled_group_mean(skill, mask, group_ids, num_groups)
    pooled_spread, _ = pooled_group_mean(spread, mask, group_ids, num_groups)
    change_norm, _ = pooled_group_mean(
        smoothed_norm(now - real_next), mask, group_ids, num_groups
    )
    ratio = score / scales
    return dict(
        loss=_over_present_groups(ratio, count),
        ratio=ratio,
        copy_ratio=change_norm / scales,
        change_norm=change_norm,
        # 1 is calibrated dispersion, 0 is a sampler that ignores its noise.
        spread_over_skill=pooled_spread / jnp.maximum(pooled_skill, 1e-12),
    )
