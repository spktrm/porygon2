...
`Porygon2OfflineCritic` = the player model's `encoder` + `v_head` under the
same `get_player_model_config`, with matching flax submodule names. Loss is
softmax cross-entropy of the 3-bin categorical value head (`[-1, 0, 1]`
support) against the final outcome at every pre-terminal step. Config is
...
overlays the trained `encoder`/`v_head` subtrees onto the fresh init;
`pi_head`/`macro_head` keep their fresh initialization. To pin warm-started
subtrees during RL, add them to `player_frozen_param_patterns` (e.g.
`("encoder/history_encoder",)`) — matching leaves get their optimizer
updates zeroed.
...
Φ(s) ∈ [-1, 1] per batch after projecting observations to the critic's
public-only training view (`mask_to_public_view`). Φ feeds the potential
advantage channel in `compute_player_targets`, gated by
