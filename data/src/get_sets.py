...
# ------------------------------------------------------------
# Core logic: JSON set ➜ packed rows
# ------------------------------------------------------------
        ...
        for _set_name, setdata in setdict.items():
            if len(ability_cols) > 0:
                if setdata.get("ability") is None:
                    setdata["ability"] = row[ability_cols].dropna(axis=1).values.squeeze().tolist()
            ...
            nature_opts  = ["" if toid(n) == "serious" else toid(n)
                            for n in expand_option(setdata.get("nature", ""))] or [""]
            ability_opts = [toid(a) for a in expand_option(setdata.get("ability", ""))] or [""]
...
# ------------------------------------------------------------
# Crawl Smogon‑format repository (pkmn.github.io) for every tier file
# ------------------------------------------------------------
...
def list_remote_jsons() -> List[str]:
    ...
    tiers = ["ubers", "ou", "uu"]
    return [f"gen{generation}{tier}.json" for generation in range(1, 9) for tier in tiers]
