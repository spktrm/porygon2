...
def _legal(cells, rows=1):
    legal = np.zeros((rows, NUM_ACTION_CELLS), bool)
    legal[:, cells] = True
    return jnp.asarray(legal)


def _loss(legal, tau=TAU, tau_max_mass=TAU_MAX_MASS):
    def loss(logits):
...
def test_silent_above_tau_and_positive_on_an_abandoned_cell():
    legal = _legal(FOUR)
...
def test_coefficient_zero_and_forced_rows_are_exactly_off():
...
def test_illegal_cells_are_never_scored():
    legal = _legal(FOUR)
...
def test_derivative_is_active_fraction_pi_minus_below_over_n_and_zero_sum():
    legal = _legal(FOUR)
...
def test_the_hinge_takes_the_zero_subgradient():
...
def test_feasibility_clamp_binds_on_a_wide_row():
...
def test_saturating_row_is_finite_and_bounded():
...
def test_batched_rows_are_independent(rows):
