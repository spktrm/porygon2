...
def _loss(y, z, legal, adv, beta):
    return hierarchical_neurd(
        macro_logits=y, micro_logits=z, adv=adv, legal=legal,
        modality_oh=MODALITY_OH, beta=beta,
    )
...
def test_gradient_is_minus_weight_per_level():
    ...
    np.testing.assert_allclose(np.asarray(gy), -np.asarray(out.macro_weight), rtol=1e-5, atol=1e-6)
    np.testing.assert_allclose(np.asarray(gz), -np.asarray(out.micro_weight), rtol=1e-5, atol=1e-6)
    ...
    for r in range(y.shape[0]):
        for m in range(3):
            ...
            np.testing.assert_allclose(
                np.asarray(out.macro_adv[r, m]), (p * adv_np[r, cells]).sum(), rtol=1e-5, atol=1e-6
            )
            np.testing.assert_allclose(
...
def test_no_pi_prefactor_on_a_starved_modality():
    ...
    np.testing.assert_allclose(np.asarray(out.macro_weight[0]), [-0.15, 0.15, 0.0], atol=1e-6)
...
def test_composed_log_policy_form_has_the_cross_term():
    ...
    def composed_log_pi(y_, z_):
        ...
        zm = jnp.where(legal[..., :, None] & jnp.asarray(MODALITY_OH), z_[..., :, None], -1e9)
    ...
    def log_pi_loss(y_, z_):
        ...
        w_cells = w_micro + (w_macro / jnp.maximum(
            legal.astype(jnp.float32) @ jnp.asarray(MODALITY_OH, jnp.float32), 1.0
        ))[..., mod_index]
