...
// were then silently lost for the rest of the run. Every await on a
// battle is now bounded and every failure reaches the client as an
// ErrorResponse.
// 10 min, not 2: a step legitimately stalls for the learner's lattice
// precompile (actors blocked behind the GPU lock). The watchdog is for
