...
from ml.config import PPOConfig
from ml.learner import PPOLearner
from ml.utils import Params, get_most_recent_file
from rlenv.client import BatchCollector
from rlenv.data import EVALUATION_SOCKET_PATH
...
def main():
    ...
    training_collector = BatchCollector(
        network,
        EVALUATION_SOCKET_PATH.replace("evaluation", "evaluation1"),
        batch_size=13,
    )
    evaluation_collector = BatchCollector(
        network,
        EVALUATION_SOCKET_PATH.replace("evaluation", "evaluation2"),
        batch_size=13,
    )
    learner = PPOLearner(network, config=learner_config)
